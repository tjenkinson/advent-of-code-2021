...
const entries = input.split('\n').filter(Boolean).map((line) => {
  ...
  return [ uniquePatternStr.split(' ').filter(Boolean), outputValueStr.split(' ').filter(Boolean)];
});

const digitByLength = new Map();
digitByLength.set(2, 1);
digitByLength.set(4, 4);
digitByLength.set(3, 7);
digitByLength.set(7, 8);

const answer = entries.reduce((acc, [, valueChars]) => {
  return acc + valueChars.filter((char) => digitByLength.has(char.length)).length;
}, 0);
