...
const entries = input.split('\n').filter(Boolean).map((line) => {
  const [uniquePatternStr, outputValueStr] = line.split('|');
  return [ uniquePatternStr.split(' ').filter(Boolean).map((a) => new Set(a.split(''))), outputValueStr.split(' ').filter(Boolean).map((a) => new Set(a.split('')))];
});

const allSegments = new Set([ 'a', 'b', 'c', 'd', 'e', 'f', 'g' ]);

const digitSegments = new Map();
digitSegments.set(0, new Set(['a', 'b', 'c', 'e', 'f', 'g']));
digitSegments.set(1, new Set(['c', 'f']));
digitSegments.set(2, new Set(['a', 'c', 'd', 'e', 'g']));
digitSegments.set(3, new Set(['a', 'c', 'd', 'f', 'g']));
digitSegments.set(4, new Set(['b', 'c', 'd', 'f']));
digitSegments.set(5, new Set(['a', 'b', 'd', 'f', 'g']));
digitSegments.set(6, new Set(['a', 'b', 'd', 'e', 'f', 'g']));
digitSegments.set(7, new Set(['a', 'c', 'f']));
digitSegments.set(8, new Set([ 'a', 'b', 'c', 'd', 'e', 'f', 'g' ]));
digitSegments.set(9, new Set([ 'a', 'b', 'c', 'd', 'f', 'g' ]));

const intersect = (a, b) => {
  const res = new Set();
  [...a].forEach((value) => {
    if (b.has(value)) res.add(value);
  });
  return res;
};
...
const setsEqual = (a, b) => {
  ...
  return [...a].every((aValue) => b.has(aValue));
};

const couldMatch = (brokenDigitSegments, correctDigitSegments, brokenSegmentToPossibleCorrectSegments) => {
  if (brokenDigitSegments.size !== correctDigitSegments.size) return false;

  const correctSegmentLit = new Map();
  const brokenSegmentLit = new Map();
  allSegments.forEach((segment) => {
    correctSegmentLit.set(segment, correctDigitSegments.has(segment));
    brokenSegmentLit.set(segment, brokenDigitSegments.has(segment));
  });

  return [...allSegments].every((segment) => {
    const brokenLit = brokenSegmentLit.get(segment);
    const possibleCorrectSegments = brokenSegmentToPossibleCorrectSegments.get(segment);
    return [...possibleCorrectSegments].some((correctSegment) => correctSegmentLit.get(correctSegment) === brokenLit);
  });
};
...
const results = entries.map(([uniquePattern, outputValue]) => {
  ...
  const brokenSegmentToPossibleCorrectSegments = new Map();
  allSegments.forEach((segment) => {
    brokenSegmentToPossibleCorrectSegments.set(segment, new Set(allSegments));
  });
    ...
    uniquePattern.forEach((brokenSegments) => {
      ...
      if (number !== null) {
        ...
        allSegments.forEach((segment) => {
          ...
          if (brokenSegments.has(segment)) {
            brokenSegmentToPossibleCorrectSegments.set(segment, intersect(possibleCorrect, correctSegments));
          } else {
            correctSegments.forEach((correctSegment) => {
              possibleCorrect.delete(correctSegment);
            });
          }
        });
      }
  ...
  const numberStr = outputValue.map((digit) => {
    for (const [key, value] of digitToNumber) {
      if (setsEqual(key, digit)) {
        return value;
      }
    }
